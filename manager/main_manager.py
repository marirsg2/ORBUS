...
class Manager:
    ...
    def IMPORTANT_get_plans_for_round(self, num_plans=30, use_gain_function=True, include_feature_distinguishing=True,
        ...
        with CodeTimer():
            ...
            results = p.map(self.parallel_prediction_computation,all_parallel_params)  # Note the last parameter is a LIST, each entry is for a new process
            ...
            for single_idx_and_result in zip(available_indices, results):
                ...
                preference_variance, preference_mean = single_result
                composite_func_integral = 0.0  # legacy need
        ...
        #     beta = 0.5  # Beta penalizes precision (variance), is chosen such that recall is considered β times as important as precision
        ...
        #     gain_normalizing_denom = np.max(gain_array)
        #
        #     gain_array += INFINITESIMAL_VALUE #Important to do this after normaliz denom calc
        #     if gain_normalizing_denom == 0.0 or math.isnan(gain_normalizing_denom):
        #         gain_normalizing_denom = 1.0  # avoids "nan" problem
        #     norm_gain_array = gain_array / gain_normalizing_denom  # normalize it
        #
        #
        ...
        #     if var_normalizing_denom == 0.0 or math.isnan(var_normalizing_denom):
        #         var_normalizing_denom = 1.0  # avoids "nan" problem
        #     norm_variance_array = variance_array / var_normalizing_denom  # normalize it
        #
        #     beta = 2
        #     # beta = 1/confidence_measure_error  # Beta penalizes precision(var), is chosen such that recall is considered β times as important as precision
        #     # F_beta = (1 + beta ** 2) * precision_variance * recall_gain / (beta ** 2 * precision_variance + recall_gain)
        #     base_score = [ (1 + beta ** 2) * norm_variance_array[x] * norm_gain_array[x] / (beta ** 2 * norm_gain_array[x] + norm_variance_array[x])
        #                             for x in range(len(norm_gain_array))]
        #     list_variance = list(variance_array)
        #     base_copy = copy.deepcopy(base_score)
        #     if not use_gain_function:
        #         while len(list_variance) > 0:
        #             a_idx = list_variance.index(max(list_variance))
        #             b_idx = base_copy.index(max(base_copy))
        #             if a_idx != b_idx:
        #                 raise Exception("Order off")
        #             del list_variance[a_idx]
        #             del base_copy[a_idx]
        #     #end else
        #     unaltered_gain_array = list(copy.deepcopy(gain_array))
        #     unaltered_variance_array = list(copy.deepcopy(variance_array))
        ...
        if include_feature_distinguishing:
            ...
            for x in range(len(index_value_list)):
                if len(index_value_list[x][
                           -1]) > 0:  # there are features in this plan, some plans have no discovered relevant features. This check avoids div by 0
        ...
        if include_probability_term:
            ...
            addendum = [addendum[x] + base_score[x] * self.compute_prob_set(index_value_list[x][-1]) for x in
    ...
    @staticmethod
    def parallel_prediction_computation(input_list):
        """
...
#=============================================================================
    def region_based_evaluation(self, annotated_test_plans, eval_output_regions=[0.2,0.8],
        ...
        # end for loop
        print(" If inside INTERESTING REGION is ", ONLY_inside_region)
        if count_samples == 0:
